...
5. **Pick the agent slug** matching the work: `des` (Design Agent / art direction), `graphic` (Graphic Agent), or `developer` (Dev Agent). Default to the task's `assigned_agent` only when it is one of those supported slugs. Pass an honest `runner` for the current environment (for example `"codex"` or `"claude"`) plus the `model` id when known.

...
Tools come from the `superhuman_operations` MCP server (already in your Codex config): `get_task`, `list_agent_queue`, `post_agent_artifact`, `prepare_agent_attachment_upload`, `finalize_agent_attachment`.
...
3. **Assemble the deliverable from this conversation:**
   - Text/research/doc → `post_agent_artifact` with `format: "html"` (self-contained doc, inline CSS) or `"markdown"`.
   - Work done in Figma → `format: "figma"` + `figma_url`.
   - Image / binary files (logo concepts, PDFs) → `prepare_agent_attachment_upload` (per file) → PUT the bytes → `finalize_agent_attachment`.

4. **Write the completion comment.** FIRST PERSON ("I" / "aku"), 2-3 sentences, **decisions-first** (what was decided / changed and why), and `<mention>Name</mention>` the right person (assignee/PM on a fresh task, `revision_requested_by` on a revision). Never third-person self-reference.

5. **Pick the agent slug** matching the work: `des` / `research` / `strategic` / `developer` / `graphic`. Default to the task's `assigned_agent`. Pass `runner: "codex"` and the `model` id for honest provenance.
...
- A **Codex Automation** is sandboxed (no storage DNS, read-only FS), so the image/file upload path (`prepare_agent_attachment_upload` → PUT → `finalize_agent_attachment`) **will fail there**. Posting an HTML/markdown artifact via `post_agent_artifact` sends content inline and works fine. For deliverables that upload an image, run `/post-comment` in an **interactive Codex chat**, not an Automation.
...
- `client_review` means "ready for the PM/founder to review", NOT "sent to the client".
